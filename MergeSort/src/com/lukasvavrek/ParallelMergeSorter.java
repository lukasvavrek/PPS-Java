    ...
    private void parallelMergeSort(float[] a, int from, int to, int availableThreads){
        if (to - from > 0){
            if (availableThreads <=1) {
                ...
                Thread firstHalf = new Thread(() -> parallelMergeSort(a, from, middle, availableThreads - 1));
                Thread secondHalf = new Thread(() -> parallelMergeSort(a, middle + 1, to, availableThreads - 1));
